/**
 * Tool registration, factored out so both the stdio entrypoint (`server.ts`)
 * and the HTTP entrypoint (`http.ts`) build an identical server over a shared
 * engine.
 *
...
/** Build a fully-wired MCP server backed by the given engine. */
